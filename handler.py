...
class ViolationHandler:
    ...
    async def handle_violation(
        ...
        if self.config.get("enable_warn", True):
            await self._send_warning(message, violation_record.violation_count, detection_result)
        ...
        if self.config.get("enable_kick", False):
            ...
            if violation_record.violation_count >= warn_threshold:
                await self._kick_user(message, user_id)
    ...
    async def _send_warning(
        ...
        try:
            base_warn = self.config.get("warn_message", "检测到您发送了广告内容，请遵守群规！")
            full_msg = f"{base_warn}\n违规原因：{detection_result.reason}\n当前违规次数：{violation_count}"
            await message.reply(full_msg)
        except Exception:
            pass

    async def _kick_user(self, message: Message, user_id: str):
        try:
